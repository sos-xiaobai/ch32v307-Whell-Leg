...
#if USART5_ENABLE
uint8_t usart5SendBuf[USART_SEND_BUF_SIZE + 1];
uint8_t usart5RecvBuf[USART_RECV_BUF_SIZE + 1];
RingBufferTypeDef usart5SendRingBuf;
RingBufferTypeDef usart5RecvRingBuf;
Usart_DataTypeDef usart5;
#endif
...
#if USART5_ENABLE
    // 赋值结构体usart指针
    usart5.pUSARTx = UART5;
    // 初始化缓冲区(环形队列)
    RingBuffer_Init(&usart5SendRingBuf, USART_SEND_BUF_SIZE, usart5SendBuf);
    RingBuffer_Init(&usart5RecvRingBuf, USART_RECV_BUF_SIZE, usart5RecvBuf);
    usart5.recvBuf = &usart5RecvRingBuf;
    usart5.sendBuf = &usart5SendRingBuf;
